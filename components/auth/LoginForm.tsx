...
export default function LoginForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use SuperTokens SDK for signin
      ...
      if (response.status === 'OK') {
        ...
        setError(response.reason || 'Sign in is not allowed');
      } else {
        setError('Invalid credentials. Please try again.');
      }
    } catch (err) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const backendUrl = process.env.NEXT_PUBLIC_API_DOMAIN || 'http://localhost:4000';
    ...
    <div className="w-full max-w-md mx-auto">
      <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end">
            <Link 
              ...
              className="text-sm text-[#FF9634] hover:text-[#E9414C] font-medium transition-colors"
            >
              Forgot password?
            </Link>
          </div>
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full px-6 py-3 bg-white border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
        >
        ...
        {/* Sign Up Link */}
        {showSignupLink && userType === 'client' && (
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600">
              Don't have an account?{' '}
              <Link 
                href="/signup" 
                className="text-[#FF9634] hover:text-[#E9414C] font-semibold transition-colors"
              >
                Get Started
              </Link>
            </p>
