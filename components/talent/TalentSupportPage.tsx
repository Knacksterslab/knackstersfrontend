...
import { talentSupportApi } from '@/lib/api/client'
import {
  Send,
  AlertCircle,
  CheckCircle,
...
export default function TalentSupportPage() {
  ...
  useEffect(() => {
    talentSupportApi.getTickets().then((res: any) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await talentSupportApi.createTicket({
              ...
              <div className="border-t border-gray-100">
                {ticketsLoading ? (
                  <div className="p-6 text-center">
                    <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-orange-400 mx-auto" />
                  </div>
                ) : tickets.length === 0 ? (
