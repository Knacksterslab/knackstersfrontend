...
import { Calendar, Clock, User, Filter, Plus, ArrowLeft } from 'lucide-react'
...
export default function MeetingsListPage() {
  ...
  const openReschedule = (e: React.MouseEvent, meeting: any) => {
    e.stopPropagation()
    setSelectedMeeting(meeting)
    setShowReschedule(true)
  }

  const openCancel = (e: React.MouseEvent, meeting: any) => {
    e.stopPropagation()
    setSelectedMeeting(meeting)
    setShowCancel(true)
  }
  ...
  const handleRescheduleComplete = (_details: BookingDetails) => {
    setShowReschedule(false)
    setSelectedMeeting(null)
    fetchMeetings()
  }
  ...
  const handleCancelMeeting = async () => {
    ...
    const result = await meetingsApi.cancel(selectedMeeting.id)
    if (result.success) {
      setSelectedMeeting(null)
      await fetchMeetings()
    } else {
    ...
    <>
            ...
            {filteredMeetings.map((meeting) => (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  ...
                  {/* Actions — stop propagation so clicks don't navigate to detail */}
                  {meeting.status === 'SCHEDULED' && new Date(meeting.scheduledAt) >= new Date() && (
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button
                        onClick={(e) => openReschedule(e, meeting)}
                      ...
                      <button
                        onClick={(e) => openCancel(e, meeting)}
      ...
      {selectedMeeting && (
        <CancelBookingDialog
          ...
          bookingDetails={{
            startTime: typeof selectedMeeting.scheduledAt === 'string'
              ? selectedMeeting.scheduledAt
              : selectedMeeting.scheduledAt.toISOString(),
            bookingId: selectedMeeting.googleCalendarEventId ?? selectedMeeting.id,
          }}
